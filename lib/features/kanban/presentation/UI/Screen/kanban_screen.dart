...
void _addTaskDialog({TaskModel? task}) {
  final isEditing = task != null;
  final titleController = TextEditingController(text: task?.title);
  final descController = TextEditingController(text: task?.description);
  
  // Initialize status with the existing task's status, or default to 'todo'
  TaskStatus? selectedStatus = task?.status ?? null; 

  // Define the blue theme color
                  ...
                  DropdownButtonFormField<TaskStatus>(
                    ...
                    items: TaskStatus.values.map((TaskStatus status) {
                      return DropdownMenuItem<TaskStatus>(
                        value: status,
                        // Keep status-specific colors here for functional clarity
                        child: Text(
                          _statusTitle(status),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            color: _statusColor(task.status), // Highlight on hover
...
Widget _buildColumn(TaskStatus status, List<TaskModel> tasks) {
  final statusTasks = tasks.where((t) => t.status == status).toList();
  final color = _statusColor(status);
  final title = _statusTitle(status);

  return Expanded(
    child: DragTarget<TaskModel>( // <--- WRAPS THE ENTIRE COLUMN'S CONTENT
      onAccept: (t) {
      ...
      builder: (context, candidateData, rejectedData) {
        // You can use the candidateData to highlight the drop zone
        final isDropping = candidateData.isNotEmpty;

        return Container(
          margin: EdgeInsets.all(5.w),
          decoration: BoxDecoration(
            color: isDropping ? color.withOpacity(0.1) : Colors.grey.shade50, // Highlight on hover
            borderRadius: BorderRadius.circular(10.r),
            border: isDropping ? Border.all(color: color, width: 2.w) : null,
          ),
          child: Column(
            children: [
              // Header (Existing code for header)
              Container(
                padding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 8.h),
                decoration: BoxDecoration(
                  color: color,
                  borderRadius: BorderRadius.vertical(top: Radius.circular(10.r)), // Changed to 10.r for consistency with column border
                ),
                child: Row(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        SizedBox(width: 5.w), // Increased width for better spacing
                        Text(
                          title,
                          style: TextStyle(color: Colors.white, fontSize: 12.sp, fontWeight: FontWeight.bold), // Increased font size
                        ),
                      ],
                    ),
                  ],
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    return LongPressDraggable<TaskModel>(
                      data: task,
                      // The 'feedback' is what follows your finger during the drag
                      feedback: Material(
                        shape: RoundedRectangleBorder(
    borderRadius: BorderRadius.circular(16.r), // Match the Container's radius
  ),
                        elevation: 4,
                        child: Container(
                          width: 100.w, // Slightly wider feedback
                          padding: EdgeInsets.all(12.w),
                          decoration: BoxDecoration(
                      ...
                      // The 'child' is the original item remaining in place
                      child: _buildTaskCard(task),
                      // The 'childWhenDragging' is what replaces the original item
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color.fromARGB(255, 221, 235, 248),
        elevation: 1,
        title:  Text(S.of(context).appTitle, style: TextStyle(color: Colors.black)),
        actions: [
         InkWell(
          onTap: (){
              ...
              style: TextStyle(
                ...
                color: Color(0xFF1E88E5), 
                 ...
                 actions: [
                   TextButton(
                onPressed: () => context.pop(),
                child: Text(S.of(context).no, style: TextStyle(color: Colors.red)),
              ),
               TextButton(
                onPressed: () {
                ...
                child: Text(S.of(context).yes, style: TextStyle(color: Colors.green)),
              )
                 ],
              );
            },);
          ...
          child: Icon(
            Icons.logout, size: 20.sp, color: Colors.red)),
      ...
      body: BlocBuilder<TaskBloc, TaskState>(
        builder: (context, state) {
          if (state is TaskLoading) {
            ...
            return Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: TaskStatus.values.map((status) => _buildColumn(status, tasks)).toList(),
            );
